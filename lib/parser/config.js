...
var CONFIG_FILTERS = [
{
    ...
    VERSION_STATIC_MODE:function(v){
        return this._formatNumber(v,0,1,v)||0;
    }
},{
    ...
    NEJ_DIR:function(v){
        ...
        return this._formatWithWebRoot(arr[0]);
    },
    ...
    NEJ_MODULE_VERSION:function (v) {
        return this._formatNumber(v,0,1,0);
    },
    ...
    OPT_IMAGE_BASE64:function(v){
        return this._formatNumber(v,0,1000000,0);
    },
    OPT_IMAGE_SPRITE:function(v){
        if (!!v){
            ...
            if (v.indexOf(this.get('DIR_WEBROOT'))!==0){
                this.emit('warn',{
                    data:[v],
                    field:'OPT_IMAGE_SPRITE',
                    message:'OPT_IMAGE_SPRITE[%s] not in DIR_WEBROOT'
                });
            }
            return v;
        }
 ...
 * @param  {String} value - path value
 * @return {String} path after variables replaced
 */
pro._formatPathVar = (function(){
    var reg = /\[(.+?)\]/gi;
    return function(value){
        var time = new Date();
        return (value||'').replace(reg,function($1,$2){
            if ($2.toLowerCase()==='timestamp'){
                return +time;
            }
            return _util.datetime(time,$2);
        });
    };
})();